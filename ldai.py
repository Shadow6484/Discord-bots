...
import sys          #NOT USED YET, BUT WILL BE USED IN AN UPCOMING UPDATE
#import pyautogui    #IMPORTED TO "PASTE" AND HIT ENTER TO SEND THE MESSAGES AUTOMATICALLY
...
@client.command()
async def speak(ctx):
        pathExists = os.path.exists('/etc/drunkbot/')
        if pathExists == False:
            print("There is no /etc/drunkbot folder, verify that the bot is installed correctly")
            sys.exit()
        try:
            print("DrunkBot Starting...")
            ...
            try:
                try:
                    with open("/etc/drunkbot/pnoun.txt") as proNouns:
                ...
                try:
                    with open("/etc/drunkbot/nouns.txt") as noun:
                        for line in noun:
                            line.rstrip()
                            nouns.append(line)
                except:
                ...
                try:
                    with open("/etc/drunkbot/verbs.txt") as verb:
                        for line in verb:
                            line.rstrip()
                            verbs.append(line)
                except:
                ...
                try:
                    with open("/etc/drunkbot/adj.txt") as adjective:
